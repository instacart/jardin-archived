...
class TestResetSession(unittest.TestCase):
    ...
    def test_reset_session_does_nothing_when_only_one_db_url_available(self):
        ...
        client = ClientProvider('jardin_test').current_client()
        jardin.reset_session()
        client2 = ClientProvider('jardin_test').current_client()
    ...
    def test_reset_session_picks_a_different_client_when_multiple_db_urls_available(self):
        ...
        client = ClientProvider('multi_url_test').current_client()
        jardin.reset_session()
        client2 = ClientProvider('multi_url_test').current_client()
    ...
    def test_reset_session_is_isolated_by_thread(self):
        ...
        before = ClientProvider('multi_url_test').current_client()
        thread = Thread(target=jardin.reset_session)
        thread.start()
        thread.join()
        after = ClientProvider('multi_url_test').current_client()
